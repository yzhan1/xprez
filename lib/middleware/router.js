...
const configureRouter = (app, folderPath) => {
  app.use(morgan('common'));

  const router = express.Router();
  const routes = require(path.join(folderPath + '/routes'))(app);

  Object.keys(routes).forEach(([key, value]) => {
    const method = value;
...
const setRoute = (router, action, endpoint, callback) => {
  switch (action) {
    ...
    case 'patch':
      ...
      break;
    default:
      throw 'InvalidHTTPMethodError';
  }
};

module.exports = (app, folderPath) => configureRouter(app, folderPath);
